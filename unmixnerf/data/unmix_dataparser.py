...
@dataclass
class unmixDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: unmixDataParser)
...
class unmixDataParser(DataParser):
    """Nerfstudio DatasetParser"""

    config: unmixDataParserConfig
    downscale_factor: Optional[int] = None

    def _generate_dataparser_outputs(self, split="train"):
        assert self.config.data.exists(), f"Data directory {self.config.data} does not exist."

        if os.path.exists(f"vca_{self.config.num_classes}.npy"):
            os.remove(f"vca_{self.config.num_classes}.npy")
